...
public class AbstractCrossLinkedListGraph<T> implements Graph<T> {
    ...
    public boolean removeVertex(T vertex) {
        var vertexNode = vertexes.get(getVertexIndex(vertex));
    ...
    public boolean removeEdge(T startVertex, T endVertex) {
        if (containsEdge(startVertex, endVertex)) {
            ...
            var out = vertexes.get(startIndex).getFirstOut();
            ...
            var in = vertexes.get(endIndex).getFirstIn();
            EdgeNode preIn = null;
            while (in != null) {
    ...
    public int getEdgeSize() {
        ...
        for (VertexNode<T> vertex : vertexes) {
            var firstOut = vertex.getFirstOut();
            while (firstOut != null) {
                size++;
